...
/**
 * A controller that creates {@link ModelProperty} for the game.<br>
...
final class LocalSetupController implements Controller {
	...
	@FXML
	private ChoiceBox<Option<Ai>> mrXAi;
	@FXML
	private ChoiceBox<Option<Ai>> detectivesAi;

	// players config tab
	@FXML
	private GridPane playerEditor;
	@FXML
	private TableView<PlayerProperty<? super Piece>> playerTable;
	@FXML
	private TableColumn<PlayerProperty<Piece>, Boolean> enabled;
	@FXML
	private TableColumn<PlayerProperty<Piece>, Piece> colour;
	@FXML
	...
	private void bindPlayerConfig(PlayerProperty<? extends Piece> previous,
		...
		if (current == null)
			return;
		if (previous != null)
			previous.observables().forEach(Property::unbind);
	...
	private void bindPlayerLocation(PlayerProperty<? extends Piece> current) {
		...
		List<Integer> locations = new ArrayList<>(current.piece() == MrX.MRX
				? ScotlandYard.MRX_LOCATIONS
				: ScotlandYard.DETECTIVE_LOCATIONS);
