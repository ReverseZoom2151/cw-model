...
/**
 * Controller for move hints with highlighting.<br>
 * Not required for the coursework.
 */
@BindFXML("layout/MoveHint.fxml")
final class MoveHintController implements Controller {
	...
	MoveHintController(ResourceManager manager, MapController board,
		...
		piece.setOpacity(0);
		Platform.runLater(() -> {
			var ms = Utils.scale(board.coordinateAtNode(source).distance(board.coordinateAtNode(target)),
