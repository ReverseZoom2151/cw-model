...
/**
 * Controller for travel log the records Mr.X move.<br>
...
public final class TravelLogController implements Controller, GameControl {
	...
	@FXML
	private TableView<RoundEntry> logTable;
	@FXML
	private TableColumn<RoundEntry, RoundEntry> logRound;
	@FXML
	private TableColumn<RoundEntry, Ticket> logTicket;
	@FXML
	private TableColumn<RoundEntry, String> logLocation;
	...
	TravelLogController(ResourceManager manager) {
		...
		logRound.setCellFactory(l -> new TableCell<>() {
			@Override
			protected void updateItem(RoundEntry item, boolean empty) {
		...
		logTicket.setCellValueFactory(
				param -> new SimpleObjectProperty<>(param.getValue().entry.map(LogEntry::ticket).getOrNull()));
	...
	@Override
	public void onGameAttach(Model model, ModelProperty config,
