...
public class ModelProperty {
	...
	private final ObjectProperty<Option<Ai>> mrXAi = new SimpleObjectProperty<>(Option.none());
	private final ObjectProperty<Option<Ai>> detectivesAi = new SimpleObjectProperty<>(Option.none());

	private final ObservableList<PlayerProperty<? super Piece>> players = FXCollections.observableArrayList();
	private final ObjectProperty<ImmutableValueGraph<Integer, ImmutableSet<Transport>>> graph = new SimpleObjectProperty<>();
	...
	public Option<Ai> getMrXAi() {
		return mrXAi.get();
	}

	public ObjectProperty<Option<Ai>> mrXAiProperty() {
		return mrXAi;
	}

	public Option<Ai> getDetectivesAi() {
		return detectivesAi.get();
	}

	public ObjectProperty<Option<Ai>> detectivesAiProperty() {
		return detectivesAi;
	}
