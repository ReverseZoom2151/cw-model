...
public class PlayerProperty<T extends Piece> {
	...
	public PlayerProperty(T piece) {
		...
		Map<Ticket, Integer> map = new TreeMap<>(
				piece == MrX.MRX ? ScotlandYard.defaultMrXTickets() : ScotlandYard.defaultDetectiveTickets());
	...
	// return
	// configs.stream().filter(PlayerProperty::detective).collect(Collectors.toList());
	// }
	//
	// public static Optional<PlayerProperty<Piece>>
	// mrX(Collection<PlayerProperty<Piece>> configs) {
